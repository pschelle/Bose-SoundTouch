...
curl -sSL https://raw.githubusercontent.com/gesellix/Bose-SoundTouch/main/scripts/on-device-install/install.sh | sh -s -- --version 0.100.0
```
...
The JSON response should include `"version":"v0.100.0"` (or whichever
...
curl -L --fail -o soundtouch-cli \
  https://github.com/gesellix/Bose-SoundTouch/releases/download/v0.100.0/soundtouch-cli-v0.100.0-linux-armv7
chmod +x soundtouch-cli

/tmp/soundtouch-cli --version
```

Replace `v0.100.0` with the version you installed.
